                ...
                {/* {menu.length ? (
                  <ul className="flex w-full flex-col">
                    {menu.map((item: Menu, index) => {
                      // Ensure item.path is valid, or default to "/"
                      const safePath = item.path ? item.path : '/';

                      return (
                        <li
                          className="py-2 text-xl text-black transition-colors hover:text-neutral-500"
                          key={`mobileNew-${index}-${item.title || 'default'}`}
                        >
                          <Link href={safePath} prefetch={true} onClick={closeMobileMenu}>
                            {item.title || 'Untitled'}
                          </Link>
                        </li>
                      );
